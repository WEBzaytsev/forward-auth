...
-   `AUTH_DOMAIN`: Полный URL-адрес, по которому будет размещен этот сервис аутентификации (например, `http://auth.example.com`). Используется для редиректов и установки правильного домена для cookie. По умолчанию `http://auth.example.com`.
