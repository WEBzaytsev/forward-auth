...
- **User-Agent фильтр** в middleware (три политики, без env): на `POST /api/login`
  блокируются пустой UA, CLI-инструменты, сканеры и headless-браузеры; на
  forward_auth без токена — CLI и сканеры (`403`, без redirect); на остальных
  маршрутах — только сканеры. Пустой UA на `GET /` разрешён (Docker HEALTHCHECK).
  С валидным `auth-token` UA не проверяется. Дополнительный слой, не замена
  rate-limit — UA легко подделать.
