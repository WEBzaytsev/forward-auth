...
var (
	...
	authDomain   = getEnv("AUTH_DOMAIN", "http://auth.example.com")
	cookieDomain string
)
	...
	if r.Method == "POST" {
		...
		if r.FormValue("password") == password {
			...
			if err == nil && parsedOriginalRedirect.IsAbs() {
				// Если originalRedirect - абсолютный URL (например, https://app.example.com/)
				...
				// или если это был прямой заход на auth.example.com/login
