...
export function proxy(req: NextRequest) {
  ...
  if (!isValid && isBlockedUserAgent(req.headers.get("user-agent"))) {
    const ip = getClientIp(req.headers);
    console.warn(
      `[ua] blocked scanner ip=${ip} path=${req.nextUrl.pathname} ua=${truncateUserAgentForLog(req.headers.get("user-agent"))}`,
    );
